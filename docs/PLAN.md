...
**Sequencing — null collector first, tracing GC last.** Build the entire GC surface
(heap, type system, cross-module identity, host API, instructions, casts) against an
**allocate-only "null collector"** that never reclaims (heap exhaustion → limiter/`Trap`),
matching `wasmtime`'s `Collector::Null`. The whole `gc` spec suite and the host GC API can
pass under it — no tracing GC in the critical path. Then drop in the real **non-moving
mark-sweep collector at the end of the phase** as a behind-the-scenes upgrade (selected by
`Collector::Auto`/`Copying`/…; `Collector::Null` stays allocate-only, as in wasmtime). This
is why the collector subtask (#27g) is sequenced after the feature set.

**Scope**
- Heap: handle table + object headers (canonical type id, mark bit, array len); `i31` unboxed; unified heap. **Initial allocator is the null collector** (allocate-only; the mark bit + slot generation live in the header but go unused until the mark-sweep upgrade).
...
- **Type-registry reclamation (bounded growth):** the engine-wide registry is **refcounted per rec group** — a `Module`/host type holds an RAII registration handle, and a group is reclaimed (whole-group, for the internal cycles) when its count hits zero. Without this a long-lived engine that churns modules/host types leaks entries and OOMs (a process-lifetime DoS). Freed canonical-id slots recycle safely (stale id → error/trap, never aliases a new type). Tracked as #27i; verified again under the Phase-8 resource-bound gate.
...
- **Mark-sweep collector** (the end-of-phase upgrade from the null collector; no surface change): non-moving, stop-the-world. No write barriers and no refcount field — the hot store/`local` paths stay plain moves. Precise root enumeration (globals/tables/operands/locals/`exnref` payloads/host roots), unified trace through reference fields, sweep frees unmarked slots and bumps the slot generation. Collects cycles; pauses to collect (acceptable — runtime speed is secondary). `Collector::Null` keeps allocate-only semantics permanently.
- GC heap limits via the limiter.

**Done when:**
- **Interim (null collector):** the `gc` spec suite passes under allocate-only `Collector::Null` (modulo any out-of-scope interactions); the host can construct/inspect structs/arrays; cast/test/canonicalization across two modules defining the same struct agree — all *before* a tracing GC exists.
...
- **Fixed-width SIMD (`v128`)** — *Wasm 2.0.* The full 236-instruction `v128` set on the existing `Val::V128` cell: every lane shape (`i8x16`…`f64x2`), splat/extract/replace_lane, arithmetic/saturating/comparison/shift/bitwise/bitmask, narrow/widen/extend/extmul/extadd/dot/q15mulr, and all `v128.load*`/`store*` (incl. `_lane`, `_splat`, `_zero`, extend variants). Enable `WasmFeatures::SIMD`. This is breadth, not depth — generate the per-op handlers systematically. **Implementation: portable scalar lane code, not SIMD intrinsics** (ARCHITECTURE §2.7) — each op is plain safe-Rust over the 16 bytes / typed lane arrays. No `std::simd` (nightly), no hardware intrinsics, no SIMD crate: stays stable-Rust, zero-`unsafe`, deterministic (exact lane/saturation/NaN semantics on every target), and LLVM autovectorizes the clean loops for free where it can — the wasmi approach. Gate it behind a Cargo **`simd` feature** so the 236-op breadth doesn't tax the SIMD-free build's compile time / binary size.
...
**Risks:** SIMD's breadth (mechanize the handlers, lean on the spec suite). Keep the scalar lane code free of stray `unsafe`/nightly and let autovectorization stay incidental — never *depend* on it; spot-check a few hot ops with `cargo show-asm` to confirm vectors are emitted, but correctness must hold scalar-only. Memory64/multi-memory touch the hot memory path — thread the index/index-type without slowing the common single-32-bit-memory case. Relaxed-SIMD's nondeterminism choice must be documented and stable — and pinned *against* autovectorization where the vector form would diverge (NaN canonicalization, relaxed lowering). Any feature added here must also re-pass the Phase-8 gates.
