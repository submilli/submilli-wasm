...
### Operand-slot GC-root location — open decision for the tracing collector (#27g)
...
relies on for *free* precise root enumeration. That's inert today (the null collector never scans
stack roots), but a **tracing collector (#27g) must recover operand-stack roots another way**, an
open choice:
- **Runtime shadow** — a parallel per-slot marker (a `Vec<u8>` byte-shadow moved in lockstep with the
  cell stack's `copy_within`; ***not* a bitmap** — relocating arbitrary bit-ranges on every
  branch/return is shift/mask-across-words and error-prone, the wrong trade in a no-panic hot path).
  Simple and keeps the trivial memmove, but re-adds ~1 B/slot and per-push bookkeeping (≈ the ~8%
  value-stack tax §14 notes we otherwise avoid).
- **Compile-time stack maps** — per-GC-safepoint metadata keyed by `(func, ip)` describing which live
  slots are refs, emitted by the compiler from the types validation already fixes at every pc. Zero
  per-push/branch cost; roots reconstructed only at collection (the JIT/wasmtime approach). More
  machinery to build.

Settle this **before #27g** implements root enumeration, so the collector isn't written against the
old tagged-stack assumption and reworked (the "earlier phases never need rework" rule, §0). (The
8-byte-cell soundness prerequisite — no `v128` value type without the `simd` feature — needed **no**
code change: wasmparser validation already rejects it, locked by a regression test.)
...
- **Collector: non-moving, stop-the-world mark-sweep** over the handle table. Chosen over deferred reference counting because it fits this project's priorities better: *(i)* runtime speed is explicitly secondary, so stop-the-world pauses are acceptable; *(ii)* precise root enumeration — the one hard part of tracing — is **already free** for us: `Val` is a tagged enum and every root lives in an explicit, walkable structure in the `Store` (we never paid Wizard's ~8% value-stack-tagging cost because the tag *is* the runtime representation) — **note** this free-rooting property held for the tagged-`Val` operand stack; the operand stack has since become an *untyped* `Cell` stack (§7), which drops the on-stack tag, so #27g must recover operand-stack roots via the open stack-maps-vs-shadow decision recorded there (other roots — globals/tables/etc. — stay tagged and free); *(iii)* no write barriers and no inc-before-dec ordering subtleties on the hot store path; *(iv)* it **collects cycles**, which DRC leaks. The collector strategy is internal and invisible at the API boundary (`Config::collector`/`Collector` still match wasmtime).
  ...
  - **When to collect — two axes, both only at safe points** (never mid-instruction; precise rooting is free at *every* allocation point, so each one already qualifies as a safe point — no constrained collection sites; the operand-stack slice of this now needs §7's stack-maps-vs-shadow choice since that stack is untyped):
