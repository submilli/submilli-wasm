...
**The start function is the first point at which guest code executes** — before the embedder ever calls an export. Its execution is an ordinary call through the interpreter loop, so it **is subject to the same fuel / epoch / stack-size limits** as any other call (#32b — `start` routes through `Func::call` → the metered run loop, so the per-op fuel/epoch and per-call stack checks apply). **Therefore limits (fuel, epoch, `max_wasm_stack`, the `ResourceLimiter`) must be armed *before* `Instance::new`**, not just before the first export call — a hostile `start` can otherwise burn unbounded CPU or recurse. Active-segment initializers are host-side copies (not fuel/epoch-metered) but are bounds-checked and bounded by the module-size cap (#32) and the memory/table limiter (#31). A trap in `start` (or in any active-segment initializer) makes `Instance::new` return `Err`; the partially-built `Instance` handle is never returned to the embedder.

> **Failure stance — short-lived store per attempt (#32b, decided):** instantiation is *not* transactionally rolled back. Entities allocated before the failing step (memories, tables, globals, the instance record) remain in the store's arenas until the `Store` is dropped; the failed instance is unreachable via the API, but its memory is not reclaimed early. This matches wasmtime (which likewise leaves entities in the store on failed instantiation). The multi-tenant guidance is therefore to **instantiate each tenant attempt into its own short-lived `Store` and discard it on failure** — the `Store` is the isolation/resource unit, and dropping it reclaims all debris; a long-lived shared store reused across repeatedly-failing instantiations would otherwise accumulate dead entities (a DoS). Transactional rollback is deliberately *not* implemented: a `start` fn can re-enter via host functions and instantiate other modules whose `Instance` handles escaped, so post-`start` arena truncation would dangle live handles. The full threat-model write-up lives in [`SECURITY.md`](../SECURITY.md) (#36).
