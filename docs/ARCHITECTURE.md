...
- Feature set: **MVP + mutable-globals + sign-extension-ops + multi-value + reference-types + function-references + GC + exception-handling**.
...
- SIMD, threads/atomics, memory64, the component model (`wasmtime::component::*`), WASI, and module serialization/AOT (`Module::serialize`/`deserialize`). These are the **only** intentional gaps in `wasmtime` API coverage; everything we *do* implement matches `wasmtime`'s signatures. (The value model and instruction table leave room to add SIMD later.)
...
- **Fuel**: a per-`Store` counter. The compiler/loop charges fuel in **block-level batches** (one charge per straight-line run, like wasmi's injected `ConsumeFuel`) to keep overhead near zero; most instructions cost 1, structural ops 0. Depletion → `Trap::OutOfFuel` (sync) or yield (async). `set_fuel` (absolute) / `get_fuel` (remaining).
- **Epoch**: `Engine` holds an `AtomicU64` epoch. `Engine::increment_epoch()` is a plain atomic add, safe to call from a background thread/timer/signal. Each `Store` has a deadline; the loop compares at back-edges and calls. On deadline: trap, extend-and-continue, or async-yield (an `UpdateDeadline`-style result). The embedder runs the ticker (we never spawn it); a helper using a weak engine handle will be provided.
...
- **Limits**: module-level (`MemoryType`/`TableType` min/max) plus a store-level `ResourceLimiter` (`memory_growing`, `table_growing`, count caps) installed via `Store::limiter`, with a `StoreLimitsBuilder` convenience (`memory_size`, `table_elements`, `instances/tables/memories`, `gc_heap_*`, `trap_on_grow_failure`).

## 14. Garbage collection (phase 5)

- **Heap**: a handle table in the `Store`. A `GcHandle` is an index (+ generation) — not a raw pointer — so references stay stable across collection and a future moving collector remains possible. Heap holds only **structs and arrays**; `i31` and nulls are unboxed and never allocated; `externref` lives in its own arena.
- **Per-object Rust allocation, not a pre-allocated arena.** Each `struct.new`/`array.new` does a real Rust allocation (a boxed object / `Vec<Val>` for fields) owned by its handle-table slot; we do **not** reserve a GC region up front or bump-allocate into it. Rationale: *(i)* fast startup is the #1 priority — an arena pays an up-front cost (and we've banned mmap/guard pages, so a reserved `Vec` is *committed* memory) to optimize per-allocation, which is the secondary axis; *(ii)* an arena only pays off with a **moving/compacting** collector — our non-moving mark-sweep over a bump region would need its own free-list + size classes, i.e. reimplementing `malloc`, whereas Rust's allocator *is* a tuned non-moving allocator; *(iii)* pointer stability comes from the `GcHandle` (index + generation), not from owning the bytes, so there's no stability argument for an arena either; *(iv)* zero `unsafe`, and it's simpler. The accepted cost (a `malloc` per allocation, worse locality) lands on runtime throughput — secondary. This is *not* inconsistent with linear memory being a grown `Vec<u8>` (§13): linear memory is a single contiguous untyped block that wasm *mandates* be addressable as one region; GC objects are individually-typed and individually-reclaimed with no addressability requirement, so the handle-table + per-object model is the natural fit. (A moving/compacting collector with an arena is the path *if* priorities ever invert to throughput; the `GcHandle` indirection keeps that door open.)
- **Object header**: canonical type id (for casts + field layout) + collector metadata (a mark bit); arrays add `len`.
