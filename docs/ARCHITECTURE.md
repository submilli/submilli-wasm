...
- **`externref` is collected too** — that's the whole point: an `externref`'s payload is **host-owned Rust data** (a `Box<dyn Any + Send + Sync>` from `ExternRef::new`), and dropping that box runs the host type's `Drop` impl, which is the *only* mechanism that releases the memory/handle/file behind it. Mechanics:
  - **Sweep drops the box.** No write barriers, no refcount, no separate finalizer API — Rust `Drop` *is* the release hook. (The arena holds no type-id/fields, which is why it's separate from the struct/array object representation.) **Current status (#27g):** the unified trace already follows references *through* the externref and exception arenas (so a GC object reachable only via an `externref`/`exnref` survives), but **reclamation of those two arenas is a documented follow-up** — only the GC object heap (structs/arrays/extern-wrappers) is swept so far; externref boxes and exn instances are still freed wholesale at `Store::drop`.
