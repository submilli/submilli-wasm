...
impl Execution {
    ...
    pub(super) fn exec_numeric(
        ...
        match op {
            ...
            O::F32Ceil => self.f32_unop_canon(f32::ceil),
            O::F32Floor => self.f32_unop_canon(f32::floor),
            O::F32Trunc => self.f32_unop_canon(f32::trunc),
            O::F32Nearest => self.f32_unop_canon(f32::round_ties_even),
            ...
            O::F64Ceil => self.f64_unop_canon(f64::ceil),
            O::F64Floor => self.f64_unop_canon(f64::floor),
            O::F64Trunc => self.f64_unop_canon(f64::trunc),
            O::F64Nearest => self.f64_unop_canon(f64::round_ties_even),
