...
impl Execution {
    ...
    pub(super) fn exec_simd_mem(
        ...
        match s {
            S::V128Load(m) => {
                let b = self.load_n::<16>(inner, instance, m)?;
                self.push_v128(u128::from_le_bytes(b));
            }
            S::V128Store(m) => {
                let v = self.pop_v128();
                self.store_n::<16>(inner, instance, m, v.to_le_bytes())?;
            }
            S::V128Load8x8S(m) => {
            ...
            S::V128Load8Lane { mem, lane } => {
                let v = self.pop_v128();
                let b = self.load_n::<1>(inner, instance, mem)?;
                let mut a = u8x16(v);
                a[*lane as usize] = b[0];
            ...
            S::V128Load16Lane { mem, lane } => {
                let v = self.pop_v128();
                let b = self.load_n::<2>(inner, instance, mem)?;
                let mut a = u16x8(v);
            ...
            S::V128Load32Lane { mem, lane } => {
                let v = self.pop_v128();
                let b = self.load_n::<4>(inner, instance, mem)?;
                let mut a = u32x4(v);
            ...
            S::V128Load64Lane { mem, lane } => {
                let v = self.pop_v128();
                let b = self.load_n::<8>(inner, instance, mem)?;
                let mut a = u64x2(v);
            ...
            S::V128Store8Lane { mem, lane } => {
                ...
                self.store_n::<1>(inner, instance, mem, [a[*lane as usize]])?;
            }
            S::V128Store16Lane { mem, lane } => {
                let a = u16x8(self.pop_v128());
                self.store_n::<2>(inner, instance, mem, a[*lane as usize].to_le_bytes())?;
            }
            S::V128Store32Lane { mem, lane } => {
                let a = u32x4(self.pop_v128());
                self.store_n::<4>(inner, instance, mem, a[*lane as usize].to_le_bytes())?;
            }
            S::V128Store64Lane { mem, lane } => {
                let a = u64x2(self.pop_v128());
                self.store_n::<8>(inner, instance, mem, a[*lane as usize].to_le_bytes())?;
            }
            _ => return Ok(false),
        }
        Ok(true)
